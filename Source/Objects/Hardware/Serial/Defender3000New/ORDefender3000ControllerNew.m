...
#import "ORDefender3000ControllerNew.h"
...
@interface ORDefender3000ControllerNew (private)
- (void) populatePortListPopup;
@end

@implementation ORDefender3000ControllerNew

#pragma mark ***Initialization

- (id) init
{
	self = [super initWithWindowNibName:@"Defender3000New"];
	return self;
}
...
- (void) setModel:(id)aModel
{
	...
	[[self window] setTitle:[NSString stringWithFormat:@"Defender 3000 New (Unit %u)",[model uniqueIdNumber]]];
}
...
- (void) registerNotificationObservers
{
    ...
    [notifyCenter addObserver : self
                     selector : @selector(weightChanged:)
                         name : ORDefender3000NewWeightChanged
                       object : nil];

    [notifyCenter addObserver : self
                     selector : @selector(pollTimeChanged:)
                         name : ORDefender3000ModelNewPollTimeChanged
                       object : nil];

	[notifyCenter addObserver : self
                     selector : @selector(shipWeightChanged:)
                         name : ORDefender3000ModelNewShipWeightChanged
						object: model];
    ...
    [notifyCenter addObserver : self
                     selector : @selector(printIntervalChanged:)
                         name : ORDefender3000NewPrintIntervalChanged
                       object : nil];

    [notifyCenter addObserver : self
                     selector : @selector(unitsChanged:)
                         name : ORDefender3000NewUnitsChanged
                       object : nil];
 
    [notifyCenter addObserver : self
                     selector : @selector(commandChanged:)
                         name : ORDefender3000NewCommandChanged
                       object : nil];
    
    [notifyCenter addObserver : self
                     selector : @selector(tareChanged:)
                         name : ORDefender3000NewTareChanged
                       object : nil];
...
{
    [unitsPopup selectItemWithTag: [(ORDefender3000ModelNew*)model units]];
}
...
- (void) checkGlobalSecurity
{
    ...
    [gSecurity setLock:ORDefender3000NewLock to:secure];
...
- (void) lockChanged:(NSNotification*)aNote
{
    ...
    BOOL lockedOrRunningMaintenance = [gSecurity runInProgressButNotType:eMaintenanceRunType orIsLocked:ORDefender3000NewLock];
    BOOL locked = [gSecurity isLocked:ORDefender3000NewLock];
    ...
    if(lockedOrRunningMaintenance){
        if(runInProgress && ![gSecurity isLocked:ORDefender3000NewLock])s = @"Not in Maintenance Run.";
    }
...
{
    [gSecurity tryToSetLock:ORDefender3000NewLock to:[sender intValue] forWindow:[self window]];
}
...
@implementation ORDefender3000ControllerNew (private)
